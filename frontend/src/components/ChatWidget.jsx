...
export default function ChatWidget() {
        ...
        try {
            ...
            if (raw) {
                return JSON.parse(raw).map(m => ({
                    ...m,
                    time: new Date(m.time)      // restore timestamp
                }))
            }
        } catch {
            // ignore parse errors
    ...
    // Loading & error states for async fetch
    ...
    // Persist updated message history to sessionStorage
    useEffect(() => {
    ...
    // Clear any error once the user starts typing again
    useEffect(() => {
    ...
    // Send handler: appends user message, calls backend, handles reply
    const send = async () => {
        ...
        // Build payload from conversation history
